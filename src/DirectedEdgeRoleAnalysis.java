...
public class DirectedEdgeRoleAnalysis {
    ...
    CSVWriter cw;
    CSVWriter mw;
    CSVWriter[] generatedWriter = new CSVWriter[6];
    ...
    int[] idxT = new int[5]; // 7days, 1month, 3month, 6month, 1years
    String centrality;
    Long2DoubleOpenHashMap centralityMap = new Long2DoubleOpenHashMap();
    static double[] boundary_centrality;

    
    public DirectedEdgeRoleAnalysis(String read_path, String dataset, int numNodes, int maxDegree, int numEdges, String centrality){
    ...
    public void run(){
        ...
        try{
            ...
            String path = resultFolder + "/" + "tracking/";
            folder = new File(path);
            if (!folder.exists()){
                folder.mkdir();
            }
            path = resultFolder + "/" + "tracking/" + centrality +"/";
            folder = new File(path);
            if (!folder.exists()){
                folder.mkdir();
            }
            ...
            for(int i = 2, j = 0 ; i <= maxDegree * 2; i = i * 2, j++){
                path = resultFolder + "/" + "tracking/" + centrality + "/" + String.format("%04d", j) + ".csv";
            ...
            String[] result_name = new String[]{"edge-immediately", "edge-1week", "edge-1month", "edge-3month", "edge-6month", "edge-1year"};
            for(int i = 0 ; i < 6; i++){
                String name = result_name[i];
                String graphlet_role_path = resultFolder + name + ".csv";
                System.out.println(graphlet_role_path);
                generatedWriter[i] = new CSVWriter(new FileWriter(new File(graphlet_role_path), false));
                String[] motif_header = {
                    "src", "dst", 
                    "m1_1", 
                    "m2_1", "m2_2",
                ...
                generatedWriter[i].writeNext(motif_header);
                generatedWriter[i].flush();
            }

            result_name = new String[]{"edge-indegree2", "edge-indegree4", "edge-indegree8"};
            for(int i = 0 ; i < 3; i++){
                String name = result_name[i];
                String graphlet_role_path = resultFolder + name + ".csv";
                System.out.println(graphlet_role_path);
            ...
            for(int i = 0; i < boundary_centrality.length+1; i++){
                CSVWriter cw = new CSVWriter(new FileWriter( resultFolder + "/" + "tracking/" + centrality + "/" + String.format("%04d", i) + ".csv", true));
                for(int j = 2; j <= 8; j *= 2){
    ...
    public void addEdge(int v1, int v2, LocalDate timestamp) {
        ...
        for(int i = 0 ; i < idxT.length; i++){
            int currentIdx = idxT[i];
            LocalDate currentTimeStamp = timestampList[currentIdx];
            LocalDate laterTimeStamp = null;
            switch(i){
                case 0 : laterTimeStamp = currentTimeStamp.plusDays(7); break;
                case 1 : laterTimeStamp = currentTimeStamp.plusMonths(1); break;
                case 2 : laterTimeStamp = currentTimeStamp.plusMonths(3); break;
                case 3 : laterTimeStamp = currentTimeStamp.plusMonths(6); break;
                case 4 : laterTimeStamp = currentTimeStamp.plusYears(1); break;
                default : break;
            }
            while(laterTimeStamp.isBefore(timestamp)){
                printChangedRole(edgeList[currentIdx].getSource(), edgeList[currentIdx].getDestination(), i + 1);
                idxT[i]++;
                currentIdx = idxT[i];
                currentTimeStamp = timestampList[currentIdx];
                switch(i){
                    case 0 : laterTimeStamp = currentTimeStamp.plusDays(7); break;
                    case 1 : laterTimeStamp = currentTimeStamp.plusMonths(1); break;
                    case 2 : laterTimeStamp = currentTimeStamp.plusMonths(3); break;
    ...
    public void printChangedRole(int src, int dst, int type){
        long key = (((long)src) << 32) | (dst & 0xffffffffL);
        if (!edgeRoleMap.containsKey(key)){
            //System.out.println("No key");
            return;
        }
        long[] initial_role_list = initialEdgeRoleMap.get(key);
        ...
        try{ 
            ...
            for(int i = 0; i < DirectedEdgeRole.values().length; i++){
                write_line[i+2] = Long.toString(role_list[i] - initial_role_list[i]);
            }
            generatedWriter[type].writeNext(write_line);
            generatedWriter[type].flush();
        }
        catch(Exception e){
            e.printStackTrace();
        }
    ...
    /*
    public long[] edgeRole2motif(){
        long[] motifs = new long[DirectedMotifType.values().length];
        long[] validate = new long[DirectedEdgeRole.values().length];
        LongSet keys = edgeRoleMap.keySet();
        for (long key : keys){
            long[] edgeRole = edgeRoleMap.get(key);
            validate[DirectedEdgeRole.e1_1.ordinal()] += edgeRole[DirectedEdgeRole.e1_1.ordinal()];
            validate[DirectedEdgeRole.e2_1.ordinal()] += edgeRole[DirectedEdgeRole.e2_1.ordinal()];
            validate[DirectedEdgeRole.e2_2.ordinal()] += edgeRole[DirectedEdgeRole.e2_2.ordinal()];
            validate[DirectedEdgeRole.e3_1.ordinal()] += edgeRole[DirectedEdgeRole.e3_1.ordinal()];
            validate[DirectedEdgeRole.e3_2.ordinal()] += edgeRole[DirectedEdgeRole.e3_2.ordinal()];
            validate[DirectedEdgeRole.e3_3.ordinal()] += edgeRole[DirectedEdgeRole.e3_3.ordinal()];
            validate[DirectedEdgeRole.e4_1.ordinal()] += edgeRole[DirectedEdgeRole.e4_1.ordinal()];
            validate[DirectedEdgeRole.e5_1.ordinal()] += edgeRole[DirectedEdgeRole.e5_1.ordinal()];
            validate[DirectedEdgeRole.e5_2.ordinal()] += edgeRole[DirectedEdgeRole.e5_2.ordinal()];
            validate[DirectedEdgeRole.e5_3.ordinal()] += edgeRole[DirectedEdgeRole.e5_3.ordinal()];
            validate[DirectedEdgeRole.e6_1.ordinal()] += edgeRole[DirectedEdgeRole.e6_1.ordinal()];
            validate[DirectedEdgeRole.e6_2.ordinal()] += edgeRole[DirectedEdgeRole.e6_2.ordinal()];
            validate[DirectedEdgeRole.e7_1.ordinal()] += edgeRole[DirectedEdgeRole.e7_1.ordinal()];
            validate[DirectedEdgeRole.e7_2.ordinal()] += edgeRole[DirectedEdgeRole.e7_2.ordinal()];
            validate[DirectedEdgeRole.e7_3.ordinal()] += edgeRole[DirectedEdgeRole.e7_3.ordinal()];
            validate[DirectedEdgeRole.e8_1.ordinal()] += edgeRole[DirectedEdgeRole.e8_1.ordinal()];
            validate[DirectedEdgeRole.e8_2.ordinal()] += edgeRole[DirectedEdgeRole.e8_2.ordinal()];
            validate[DirectedEdgeRole.e9_1.ordinal()] += edgeRole[DirectedEdgeRole.e9_1.ordinal()];
            validate[DirectedEdgeRole.e10_1.ordinal()] += edgeRole[DirectedEdgeRole.e10_1.ordinal()];
            validate[DirectedEdgeRole.e10_2.ordinal()] += edgeRole[DirectedEdgeRole.e10_2.ordinal()];
            validate[DirectedEdgeRole.e10_3.ordinal()] += edgeRole[DirectedEdgeRole.e10_3.ordinal()];
            validate[DirectedEdgeRole.e10_4.ordinal()] += edgeRole[DirectedEdgeRole.e10_4.ordinal()];
            validate[DirectedEdgeRole.e11_1.ordinal()] += edgeRole[DirectedEdgeRole.e11_1.ordinal()];
            validate[DirectedEdgeRole.e11_2.ordinal()] += edgeRole[DirectedEdgeRole.e11_2.ordinal()];
            validate[DirectedEdgeRole.e12_1.ordinal()] += edgeRole[DirectedEdgeRole.e12_1.ordinal()];
            validate[DirectedEdgeRole.e12_2.ordinal()] += edgeRole[DirectedEdgeRole.e12_2.ordinal()];
            validate[DirectedEdgeRole.e12_3.ordinal()] += edgeRole[DirectedEdgeRole.e12_3.ordinal()];
            validate[DirectedEdgeRole.e12_4.ordinal()] += edgeRole[DirectedEdgeRole.e12_4.ordinal()];
            validate[DirectedEdgeRole.e12_5.ordinal()] += edgeRole[DirectedEdgeRole.e12_5.ordinal()];
            validate[DirectedEdgeRole.e13_1.ordinal()] += edgeRole[DirectedEdgeRole.e13_1.ordinal()];
        }

        motifs[DirectedMotifType.m1.ordinal()] = validate[DirectedEdgeRole.e1_1.ordinal()];
        motifs[DirectedMotifType.m2.ordinal()] = validate[DirectedEdgeRole.e2_1.ordinal()]
                                                + validate[DirectedEdgeRole.e2_2.ordinal()];
        motifs[DirectedMotifType.m3.ordinal()] = validate[DirectedEdgeRole.e3_1.ordinal()]
                                                + validate[DirectedEdgeRole.e3_2.ordinal()]
                                                + validate[DirectedEdgeRole.e3_3.ordinal()];
        motifs[DirectedMotifType.m4.ordinal()] = validate[DirectedEdgeRole.e4_1.ordinal()];
        motifs[DirectedMotifType.m5.ordinal()] = validate[DirectedEdgeRole.e5_1.ordinal()]
                                                + validate[DirectedEdgeRole.e5_2.ordinal()]
                                                + validate[DirectedEdgeRole.e5_3.ordinal()];
        motifs[DirectedMotifType.m6.ordinal()] = validate[DirectedEdgeRole.e6_1.ordinal()]
                                                + validate[DirectedEdgeRole.e6_2.ordinal()];
        motifs[DirectedMotifType.m7.ordinal()] = validate[DirectedEdgeRole.e7_1.ordinal()]
                                                + validate[DirectedEdgeRole.e7_2.ordinal()]
                                                + validate[DirectedEdgeRole.e7_3.ordinal()];        
        motifs[DirectedMotifType.m8.ordinal()] = validate[DirectedEdgeRole.e8_1.ordinal()]
                                                + validate[DirectedEdgeRole.e8_2.ordinal()];
        motifs[DirectedMotifType.m9.ordinal()] = validate[DirectedEdgeRole.e9_1.ordinal()];     
        motifs[DirectedMotifType.m10.ordinal()] = validate[DirectedEdgeRole.e10_1.ordinal()]
                                                + validate[DirectedEdgeRole.e10_2.ordinal()]
                                                + validate[DirectedEdgeRole.e10_3.ordinal()]
                                                + validate[DirectedEdgeRole.e10_4.ordinal()];
        motifs[DirectedMotifType.m11.ordinal()] = validate[DirectedEdgeRole.e11_1.ordinal()]
                                                + validate[DirectedEdgeRole.e11_2.ordinal()];        
        motifs[DirectedMotifType.m12.ordinal()] = validate[DirectedEdgeRole.e12_1.ordinal()]
                                                + validate[DirectedEdgeRole.e12_2.ordinal()]
                                                + validate[DirectedEdgeRole.e12_3.ordinal()]
                                                + validate[DirectedEdgeRole.e12_4.ordinal()]
                                                + validate[DirectedEdgeRole.e12_5.ordinal()];
        motifs[DirectedMotifType.m13.ordinal()] = validate[DirectedEdgeRole.e13_1.ordinal()];
    ...
    */
    ...
    public DirectedMotifType getMotifType(int v1, int v2, int v3){
        int V1IN = 0, V2IN = 0, V3IN = 0, V1OUT = 0, V2OUT = 0, V3OUT = 0;
        IntSet V1OUTSET = node_map.getOutNeighbors(v1);
        IntSet V2OUTSET = node_map.getOutNeighbors(v2);
        IntSet V3OUTSET = node_map.getOutNeighbors(v3);
